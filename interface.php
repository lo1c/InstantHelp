<?php
	if(isset($_POST['username']) && isset($_POST['password']) && isset($_POST['name']) && isset($_POST['surname']) && isset($_POST['phone']) && isset($_POST['email']) && isset($_POST['birthday'])) {
		try {
			$hey = fopen("confirmed/" . $_POST['username'], "r");
			header('Location: /register.php?message=UsernameAlreadyGiven&username=' . $_POST['username'] . "&name=" . $_POST['name'] . "&surname=" . $_POST['surname'] . "&birthday=" . $_POST['birthday'] . "&phone=" . $_POST['phone'] . "&email=" . $_POST['email']);
		} catch(Exception $ex) {
			try {
				...
				$content2 = '<?php ' . chr(36) . 'file = fopen("../confirmed/' . $_POST["username"] . '", "w"); fclose(' . chr(36) . 'file); header("location: ../login.php"); ?>';
